 ...
 * <p>
 * Validates synthetic AprilTag detection using the DECODE field layout:
 * only 2 tags (Tag 20: Blue Goal at (-1.5, -1.5), Tag 24: Red Goal at (1.5, 1.5)).
 * The Obelisk is outside the field and not simulated.
 */
public class VisionIOSimTest {
    ...
    void testRobotFacingRedGoalSeesIt() {
        ...
        assertTrue(Math.abs(inputs.tx) < 15.0,
    ...
    void testBeyondMaxRangeNoDetection() {
        ...
        assertFalse(inputs.hasTarget,
            "Goals at ~2.12m should not be detected with 0.3m max range");
    ...
    void testNoTargetDoesNotCrash() {
        ...
        robotBody.translateToOrigin();
        assertDoesNotThrow(() -> impossibleSim.updateInputs(inputs),
            "Should not crash when no tags are visible");
        assertFalse(inputs.hasTarget);
    }
}
