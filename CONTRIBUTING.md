...
- **SI Units:** Use `org.areslib.math.Units` for all unit conversions. If storing physics values in a primitive `double`, the unit suffix MUST be explicit in the variable name (e.g., `double velocityMetersPerSecond`, `double accelerationNewtons`, NOT `double vel`).
